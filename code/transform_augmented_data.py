...
def generate_file(split, fp_in, aug_dir, ontology):
    fp_out = open(get_path(aug_dir, split+".tsv"), 'w')
    ...
    for dial in fp_in:
        ...
        for t_idx, turn in enumerate(dial["dialogue"]):
            ...
            fp_out.write(dial["dialogue_idx"])                   # 0: dialogue ID
...
def main():
    ...
    ### Read ontology file
    ontology_file = get_path(args.input_dir, "multi-woz/MULTIWOZ2.1/ontology.json")
    ...
    aug_train_file = get_path(args.input_dir, "train_dials.json")
    aug_test_file = get_path(args.input_dir, "test_dials.json")
    aug_dev_file = get_path(args.input_dir, "dev_dials.json")

    aug_train = json.load(open(aug_train_file, 'r'))
    aug_test = json.load(open(aug_test_file, 'r'))
    aug_dev = json.load(open(aug_dev_file, 'r'))
